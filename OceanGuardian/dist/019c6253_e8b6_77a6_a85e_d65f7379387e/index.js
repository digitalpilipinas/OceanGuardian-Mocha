...
objectType({
  ...
  role: UserRoleSchema,
  level: numberType().int().min(1).max(50),
  xp: numberType().int().min(0),
  reputation: numberType().int().min(0),
  streak_days: numberType().int().min(0),
  streak_freezes: numberType().int().min(0),
  last_check_in: stringType().nullable(),
  total_sightings: numberType().int().min(0),
  total_missions: numberType().int().min(0),
  assigned_regions: stringType().nullable(),
...
const SightingTypeSchema = enumType(["garbage", "floating", "wildlife", "coral"]);
const SightingStatusSchema = enumType(["pending", "approved", "flagged", "removed"]);
const CreateSightingSchema = objectType({
  type: SightingTypeSchema,
  subcategory: stringType().min(1),
  description: stringType().min(1).max(500),
  severity: numberType().int().min(1).max(5),
...
objectType({
  ...
  image_key: stringType().nullable(),
  bleach_percent: numberType().nullable(),
  water_temp: numberType().nullable(),
  depth: numberType().nullable(),
  mission_id: numberType().nullable(),
  validated: numberType(),
  validation_count: numberType(),
  status: SightingStatusSchema,
  flag_reason: stringType().nullable(),
  ai_analysis: stringType().nullable(),
  created_at: stringType(),
  updated_at: stringType(),
  user_name: stringType().optional()
});
const BadgeCategorySchema = enumType(["milestone", "mission", "streak", "specialty", "community", "seasonal"]);
const BadgeRaritySchema = enumType(["common", "uncommon", "rare", "epic", "legendary"]);
objectType({
  id: numberType(),
  name: stringType(),
  description: stringType(),
  ...
  icon: stringType(),
  requirement_type: stringType(),
  requirement_value: numberType(),
  is_hidden: numberType(),
  created_at: stringType()
});
objectType({
  id: numberType(),
